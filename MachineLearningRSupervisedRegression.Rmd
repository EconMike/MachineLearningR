...
dfm1<-df%>%select(Height, Width,Length1,Length2, Length3,Weight)
set.seed(100)

boxplot(dfm1)


p1<- ggplot(dfm1, aes(y = Weight, x = Height)) +
  geom_point()
p2<- ggplot(dfm1, aes(y = Weight, x = Width)) +
  geom_point()
p3<- ggplot(dfm1, aes(y = Weight, x = Length1)) +
  geom_point()
p4<- ggplot(dfm1, aes(y = Weight, x = Length2)) +
  geom_point()
p5<- ggplot(dfm1, aes(y = Weight, x = Length3)) +
  geom_point()
...
trainSetm2 <- subset(x=dfm2, sampleSplit3==TRUE)
testSetm2 <- subset(x=dfm2, sampleSplit3==FALSE)
...
#normality test
shapiro.test(model_test3$residuals)
#homoscedasticity test -assumption of equal or similar variances [we want to reject]
bptest(model_test3)
